...
export function Explication() {
  // Déclarer un état local pour suivre l'index de l'élément ouvert
  const [openIndex, setOpenIndex] = useState(null);

  // Fonction pour basculer l'état d'ouverture de l'élément
  const toggleItem = (index) => {
    if (openIndex === index) {
      // Si l'index est déjà ouvert, le fermer
      setOpenIndex(null);
    } else {
      // Sinon, ouvrir le nouvel index
      setOpenIndex(index);
    }
  };
      ...
      <ul className="exp-list">
        {livresData.map((livre, index) => (
          <li key={index}>
            ...
            {/* Afficher le contenu si l'index correspond à celui de l'élément ouvert */}
            {openIndex === index && (
              <div className="exp">
